...
def run(_run, _config, _log):
    ...
    _log.info(f"Runner specified in config: {_config.get('runner')}")
    _log.info(f"Runner resolved in args: {args.runner}")
    ...
    _log.info(f"Final parsed config: {_config}")
...
def run_sequential(args, logger):
    ...
    logger.console_logger.info(f"run.py Using runner: {args.runner}")
    runner = r_REGISTRY[args.runner](args=args, logger=logger)
    logger.console_logger.info(f"Initialized runner: {type(runner).__name__}")
    ...
    if args.checkpoint_path != "":
        ...
        if timesteps:
            ...
            logger.console_logger.info(
                "No checkpoints found in directory; starting fresh."
            )
