...
class SimCityWrapper(MultiAgentEnv):
    def __init__(self, grid_x=4, grid_y=4, **kwargs):
        ...
        logger.debug(
            f"simcity_wrapper: Obs size={self.obs_size}, State size={self.state_size}, Actions={self.n_actions}"
        )
    ...
    def get_obs(self):
        ...
        logger.debug(
            f"simcity_wrapper: Aggregated observations shape={obs_array.shape}"
        )
        return obs_array
    ...
    def get_avail_agent_actions(self, agent_id):
        logger.debug(
            f"simcity_wrapper: Fetching available actions for agent {agent_id}."
        )
        agent = self.env.agents[agent_id]
        ...
        for action in range(self.n_actions):
            ...
            if (
                ...
                and player_resources["reputation"] >= building_cost["reputation"]
            ):
                avail_actions[action] = 1.0  # Action is available

        logger.debug(
            f"simcity_wrapper: Available actions for agent {agent_id}: {avail_actions}"
        )
    ...
    def step(self, actions):
        ...
        for idx, agent in enumerate(self.env.agents):
            if self.env.terminations[agent] or self.env.truncations[agent]:
                self.env.step(None)
                logger.debug(
                    f"simcity_wrapper: Agent {agent} is terminated or truncated; stepping with None."
                )
                continue
            ...
            if action < 0 or action >= self.n_actions:
                logger.warning(
                    f"simcity_wrapper: Invalid action {action} for agent {agent}. Forcing No-op."
                )
            ...
            if all(self.env.terminations.values()) or all(
                ...
                logger.debug(
                    "simcity_wrapper: Game ended during multi-agent step loop."
                )
                break
    ...
    def reset(self, seed=None, options=None):
        ...
        self.n_actions = self.env.action_spaces["P1"].n if self.n_agents > 0 else 0
        logger.debug(
            f"simcity_wrapper: Reset results - n_agents={self.n_agents}, obs_size={self.obs_size}, state_size={self.state_size}, n_actions={self.n_actions}"
        )
